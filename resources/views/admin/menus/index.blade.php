	...
	<div class="row page-titles mx-0 mb-3">
		<div class="col-sm-6 p-0">
			<div class="welcome-text">
				<h4>{{ __('Menus') }}</h4>
				<span>{{ __('All menus list') }}</span>
			</div>
		</div>
		<div class="col-sm-6 p-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
			<ol class="breadcrumb">
				<li class="breadcrumb-item"><a href="{{ route('menu.admin.admin_index') }}">{{ __('Menus') }}</a></li>
				<li class="breadcrumb-item active"><a href="javascript:void(0)">{{ __('All menus list') }}</a></li>
			</ol>
		</div>
	</div>
				...
				<div class="card">
					...
					<div class="card-body">
						<div class="form-group row">
							<label class="col-md-3 col-form-label">{{ __('Select a menu to edit:') }}<span class="text-danger">* </span></label>
							<div class="col-md-5">
							...
							<div class="col-md-4">
								...
								{{ __('or') }}
								<a href="javascript:void(0);" class="text-primary" id="CreateMenu">{{ __('create new menu') }}</a>
				...
				<div class="col-md-12">
					<div class="card card-menu-pages accordion accordion-rounded-stylish accordion-bordered" id="accordion-menus">
						<div class="card-header d-block bg-primary accordion-header accordion-header--primary" data-bs-toggle="collapse" data-bs-target="#rounded-menus" aria-expanded="true">
							...
							<span class="accordion-header-text">{{ __('Menus') }}</span>
							<span class="accordion-header-indicator"></span>
						</div>
								...
								<div class="accordion__item mb-2 XPages {{ $screenOption['Pages']['visibility'] ? '' : 'd-none' }}">
									<form action="{{ route('menu.admin.ajax_add_page') }}" id="MenuAjaxAddPageForm" method="post">
										...
										<div class="accordion-header accordion-header--primary" data-bs-toggle="collapse" data-bs-target="#rounded-pages" aria-expanded="true">
											<span class="accordion-header-icon"></span>
											<span class="accordion-header-text">{{ __('Pages') }}</span>
											<span class="accordion-header-indicator"></span>
										</div>
										<div id="rounded-pages" class="accordion__body ItemsCheckboxSec default-tab show p-3" data-bs-parent="#accordion-menu-child">
											<ul class="nav nav-tabs" role="tablist">
												<li class="nav-item">
													<a class="nav-link active checkboxUtility" data-bs-toggle="tab" href="#view-all-pages">{{ __('View All') }}</a>
												</li>
												<li class="nav-item">
													<a class="nav-link checkboxUtility" data-bs-toggle="tab" href="#page-search">{{ __('Search') }}</a>
												</li>
											</ul>
											<div class="tab-content border-end border-start border-bottom">
												...
												<div class="tab-pane fade" id="page-search">
													<div class="p-2 form-group">
														...
														<input type="search" class="form-control SearchForMenu" placeholder="{{ __('Enter Page Name') }}">
											...
											<div class="row">
												...
												<div class="col-md-6 text-end pt-2">
													<button type="button" class="btn btn-primary btn-xs AddToMenu" menu-type="page" menu-id="{{ optional($menu)->id }}">{{ __('Add to Menu') }}</button>
												</div>
											</div>
										</div>
									</form>
								...
								<div class="accordion__item mb-2 XBlogs {{ $screenOption['Blogs']['visibility'] ? '' : 'd-none' }}">
									<form action="{{ route('menu.admin.ajax_add_page') }}" id="MenuAjaxAddBlogForm" method="post">
										...
										<div class="accordion-header accordion-header--primary collapsed" data-bs-toggle="collapse" data-bs-target="#rounded-blogs">
											<span class="accordion-header-icon"></span>
											<span class="accordion-header-text">{{ __('Blogs') }}</span>
											<span class="accordion-header-indicator"></span>
										</div>
										<div id="rounded-blogs" class="accordion__body ItemsCheckboxSec default-tab collapse p-3" data-bs-parent="#accordion-menu-child">
											<ul class="nav nav-tabs" role="tablist">
												<li class="nav-item">
													<a class="nav-link active checkboxUtility" data-bs-toggle="tab" href="#view-all-blogs">{{ __('View All') }}</a>
												</li>
												<li class="nav-item">
											...
											<div class="row">
												...
												<div class="col-md-6 text-end pt-2">
													<button type="button" class="btn btn-primary AddToMenu btn-xs" menu-type="blog" menu-id="{{ optional($menu)->id }}">{{ __('Add to Menu') }}</button>
												</div>
											</div>
										</div>
									</form>
								...
								<div class="accordion__item mb-2 XCustomLinks {{ $screenOption['CustomLinks']['visibility'] ? '' : 'd-none' }}">
									<form action="{{ route('menu.admin.ajax_add_link') }}" id="MenuAjaxAddLinkForm" method="post" accept-charset="utf-8">
										...
										<div class="accordion-header collapsed accordion-header-info" data-bs-toggle="collapse" data-bs-target="#rounded-links">
											<span class="accordion-header-icon"></span>
											<span class="accordion-header-text">{{ __('Links') }}</span>
											<span class="accordion-header-indicator"></span>
										</div>
										<div id="rounded-links" class="collapse accordion__body p-3" data-bs-parent="#accordion-menu-child">
											...
											<div class="form-group row">
												<label class="col-sm-12 col-form-label">{{ __('Link Text') }}</label>
												<div class="col-sm-12">
													<input type="text" name="Menu[linktitle]" class="form-control MenuLinkTitle" id="MenuLinktitle" placeholder="{{ __('Menu Item') }}">
												</div>
											</div>
											<div class="form-group row">
												<div class="col-md-12 text-end">
													<button type="button" class="btn btn-primary LinksAddToMenu btn-xs" menu-type="custom-link" menu-id="{{ optional($menu)->id }}">{{ __('Add to Menu') }}</button>
												</div>
											</div>
										</div>
									</form>
								...
								<div class="accordion__item mb-2 XCategories {{ $screenOption['Categories']['visibility'] ? '' : 'd-none' }}">
									<form action="{{ route('menu.admin.ajax_add_page') }}" id="MenuAjaxAddCategoryForm" method="post">
										...
										<div class="accordion-header accordion-header--primary collapsed" data-bs-toggle="collapse" data-bs-target="#rounded-categories">
											<span class="accordion-header-icon"></span>
											<span class="accordion-header-text">{{ __('Categories') }}</span>
											<span class="accordion-header-indicator"></span>
										</div>
										<div id="rounded-categories" class="accordion__body ItemsCheckboxSec default-tab p-3 collapse" data-bs-parent="#accordion-menu-child">
											<ul class="nav nav-tabs" role="tablist">
												<li class="nav-item">
													<a class="nav-link active checkboxUtility" data-bs-toggle="tab" href="#view-all-categories">{{ __('View All') }}</a>
												</li>
												<li class="nav-item">
													<a class="nav-link checkboxUtility" data-bs-toggle="tab" href="#category-search">{{ __('Search') }}</a>
												</li>
											</ul>
											<div class="tab-content border-end border-start border-bottom">
												...
												<div class="tab-pane fade" id="category-search">
													<div class="p-2 form-group">
														<label for="">{{ __('Search') }}</label>
														<input type="search" class="form-control SearchForMenu" placeholder="{{ __('Enter Category Name') }}">
											...
											<div class="row">
												...
												<div class="col-md-6 text-end pt-2">
													<button type="button" class="btn btn-primary btn-xs AddToMenu" menu-type="category" menu-id="{{ optional($menu)->id }}">{{ __('Add to Menu') }}</button>
												</div>
											</div>
										</div>
									</form>
								...
								<div class="accordion__item mb-2 XTags {{ $screenOption['Tags']['visibility'] ? '' : 'd-none' }}">
									<form action="{{ route('menu.admin.ajax_add_page') }}" id="MenuAjaxAddTagForm" method="post">
										...
										<div class="accordion-header accordion-header--primary collapsed" data-bs-toggle="collapse" data-bs-target="#rounded-tags">
											<span class="accordion-header-icon"></span>
											<span class="accordion-header-text">{{ __('Tags') }}</span>
											<span class="accordion-header-indicator"></span>
										</div>
										<div id="rounded-tags" class="accordion__body ItemsCheckboxSec default-tab p-3 collapse" data-bs-parent="#accordion-menu-child">
											<ul class="nav nav-tabs" role="tablist">
												<li class="nav-item">
													<a class="nav-link active checkboxUtility" data-bs-toggle="tab" href="#view-all-tags">{{ __('View All') }}</a>
												</li>
												<li class="nav-item">
													<a class="nav-link checkboxUtility" data-bs-toggle="tab" href="#tag-search">{{ __('Search') }}</a>
												</li>
											</ul>
											<div class="tab-content border-end border-start border-bottom">
												...
												<div class="tab-pane fade" id="tag-search">
													<div class="p-2 form-group">
														<label for="">{{ __('Search') }}</label>
														<input type="search" class="form-control SearchForMenu" placeholder="{{ __('Enter Tag Name') }}">
											...
											<div class="row">
												...
												<div class="col-md-6 text-end pt-2">
													<button type="button" class="btn btn-primary btn-xs AddToMenu" menu-type="tag" menu-id="{{ optional($menu)->id }}">{{ __('Add to Menu') }}</button>
												</div>
											</div>
										</div>
									</form>
						...
						<div class="card card-menu-items">
						        ...
						        <div id="accordion-menu-structure" class="accordion accordion-rounded-stylish accordion-bordered">
									<div class="dd menuss" id="NestableMenu">
							            ...
							            <div class="menu-settings">
							            	...
							            	<div class="row">
							            		<div class="col-md-4">
						            				<label>{{ __('Display location') }}</label>
							            		</div>
							            		<div class="col-md-8">
													<div class="form-group">
														@forelse($menusLocations as $key => $value)
															...
															<div class="form-check  text-nowrap mb-2">
																...
																<label class="form-check-label" for="{{ $key }}">{{ $value['title'] }}{{ !empty($menusLocations[$key]['menu']) && ($menusLocations[$key]['menu'] != optional($menu)->id) ? '(used in '.$menu_name.')' : '' }}</label>
															</div>
														@empty
														@endforelse
													</div>
							...
							<div class="card-footer bg-primary d-flex align-items-center justify-content-between">
								<div class="delete-menu-btn">
									<a href="javascript:void(0)" class="DeleteMenu btn btn-danger btn-sm me-2" rel="{{ optional($menu)->id }}" menu-name="{{ optional($menu)->title }}" title="{{ __('Click here to delete menu') }}">{{ __('Delete Menu') }}</a>
								</div>
								<button type="submit" class="btn btn-success btn-sm" title="{{ __('Click here to save the menu') }}">{{ __('Save Menu') }}</button>
							</div>
						</div>
					</form>
				</div>
