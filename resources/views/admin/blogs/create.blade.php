	...
	<div class="row page-titles mx-0 ">
		...
		<div class="col-sm-6 p-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item active"><a href="javascript:void(0)">Tambah Postingan</a></li>
			</ol>
		</div>
	</div>
							...
							<div class="card-body p-4">
								<div class="row">
									<div class="form-group col-md-12">
										...
										<input type="text" name="data[Blog][title]" class="form-control" id="BlogTitle" placeholder="Judul" value="{{ old('data.Blog.title') }}">
									...
									<div class="form-group col-md-12 ">
											<textarea name="data[Blog][content]" class="form-control W3cmsCkeditor h-auto" id="BlogContent" rows="10">{{ old('data.Blog.content') }}</textarea>
					...
					<div class="col-md-12">
						<div class="card accordion accordion-rounded-stylish accordion-bordered XDiscussion {{ $screenOption['Discussion']['visibility'] ? '' : 'd-none' }}" id="accordion-discussion">
							<div class="card-header justify-content-start accordion-header" data-bs-toggle="collapse" data-bs-target="#with-discussion" aria-expanded="true">
								<h4 class="card-title">Diskusi</h4>
								<span class="accordion-header-indicator"></span>
							</div>
							...
							<div class="accordion__body p-4 collapse show" id="with-seo" data-bs-parent="#accordion-seo">
								<div class="row">
									<div class="col-md-12 form-group">
										...
										<input type="text" name="data[BlogSeo][page_title]" class="form-control" id="ContentSeoBlogTitle" placeholder="{{ __('Blog Title') }}" maxlength="255" value="{{ old('data.BlogSeo.page_title') }}">
									</div>
									<div class="form-group col-md-6">
										<label for="ContentSeoMetaKeywords">{{ __('Keywords') }}</label>
										<input type="text" name="data[BlogSeo][meta_keywords]" class="form-control" id="ContentSeoMetaKeywords" placeholder="{{ __('Enter meta keywords') }}" maxlength="255" value="{{ old('data.BlogSeo.meta_keywords') }}">
									</div>
									<div class="form-group col-md-6">
										<label for="ContentSeoMetaDescriptions">{{ __('Descriptions') }}</label>
										<textarea name="data[BlogSeo][meta_descriptions]" class="form-control" id="ContentSeoMetaDescriptions" rows="5" placeholder="{{ __('Enter meta descriptions') }}">{{ old('data.BlogSeo.meta_descriptions') }}</textarea>
									</div>
								</div>
							</div>
						</div>
								...
								<div class="row">
									<div class="col-md-12 form-group">
										...
										<select name="data[Blog][status]" id="Status" class="default-select form-control">
											<option value="1" {{ old('data.Blog.status') == 1 ? 'selected="selected"' : '' }}>{{ __('Published') }}</option>
											<option value="2" {{ old('data.Blog.status') == 2 ? 'selected="selected"' : '' }}>{{ __('Draft') }}</option>
											{{-- <option value="4" {{ old('data.Blog.status') == 4 ? 'selected="selected"' : '' }}>{{ __('Private') }}</option> --}}
											<option value="5" {{ old('data.Blog.status') == 5 ? 'selected="selected"' : '' }}>{{ __('Pending') }}</option>
										</select>
									</div>
									<div class="col-md-12 form-group">
										...
										<select name="data[Blog][visibility]" id="ContentVisibility" class="default-select form-control">
											<option value="Pu" {{ old('data.Blog.visibility') == 'Pu' ? 'selected="selected"' : '' }}>{{ __('Public') }}</option>
											{{-- <option value="PP" {{ old('data.Blog.visibility') == 'PP' ? 'selected="selected"' : '' }}>{{ __('Password protected') }}</option> --}}
											<option value="Pr" {{ old('data.Blog.visibility') == 'Pr' ? 'selected="selected"' : '' }}>{{ __('Private') }}</option>
										</select>
									</div>
									<div class="col-md-12 form-group {{ old('data.Blog.visibility') == 'PP' ? '' : 'd-none' }}" id="PublicPasswordTextbox">
										...
										<input type="password" name="data[Blog][password]" class="form-control" id="ContentPassword" placeholder="{{ __('Enter Password') }}" value="{{ old('data.Blog.password') }}" autocomplete="New-Password">
									</div>
					...
					<div class="col-md-12">
						<div class="card accordion accordion-rounded-stylish accordion-bordered XTags {{ $screenOption['Tags']['visibility'] ? '' : 'd-none' }}" id="accordion-tags">
							...
							<div class="accordion__body p-4 collapse show" id="with-tags" data-bs-parent="#accordion-tags">
								<input type="text" name="data[BlogTag]" data-role="tagsinput" class="form-control bootstrap-tagsinput" placeholder="{{ __('type tags here') }}" id="BlogBlogTag" value="{{ old('data.BlogTag') }}">
							</div>
						</div>
					</div>
					<div class="col-md-12">
					...
					<div class="col-md-12">
						<div class="card accordion accordion-rounded-stylish accordion-bordered XVideo {{ $screenOption['Video']['visibility'] ? '' : 'd-none' }}" id="accordion-video">
							...
							<div class="accordion__body p-4 collapse show" id="with-video" data-bs-parent="#accordion-video">
								...
								<input type="text" name="data[BlogMeta][1][value]" class="form-control bootstrap-tagsinput" placeholder="{{ __('Youtube Video Link') }}" id="BlogMeta1Value" value="{{ old('data.BlogMeta.1.value') }}">
							</div>
						</div>
					</div>
				</div>
