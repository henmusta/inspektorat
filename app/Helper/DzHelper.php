...
class DzHelper
{
    ...
    */
    ...
    public static function homeGalery($atts = array())
    {
    	$configs = array(
            'limit'		=> isset($atts['limit']) ? $atts['limit'] : config('Reading.nodes_per_page') ,
            'order' 	=> isset($atts['order']) ? $atts['order'] : 'desc',
		...
		$galeri = New Galeri();
		$galery = $galeri->recentGalery($configs);


        return view('widgets.galeri',  compact('galery'));
		// if (!empty(config('Widget.show_recent_post_widget'))) {

		// }
    }

    public static function recenthomeBlogs($atts = array())
    {
    	$configs = array(
            'limit'		=> isset($atts['limit']) ? $atts['limit'] : config('Reading.nodes_per_page') ,
            'order' 	=> isset($atts['order']) ? $atts['order'] : 'desc',
		...
		$blog = New Blog();
		$blogs = $blog->recenthomeBlogs($configs);

        return view('widgets.recent_blogs_home', compact('blogs'));
		// if (!empty(config('Widget.show_recent_post_widget'))) {
